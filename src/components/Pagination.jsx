...
const Pagination = ({pageNumber, paginate, next, prev, currentPage}) => {
  ...
  <ul class="inline-flex -space-x-px text-sm">
    ...
    {pageNumber.map((item,i)=>(
      ...
      <a className = {`${currentPage == i + 1 ?"flex items-center justify-center px-3 h-8 leading-tight text-white bg-[#262626] border border-gray-300 dark:border-gray-700"
        :"flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border  border-gray-300 dark:border-gray-700 "}`} >{item}</a>
    </li>
